...
declare namespace Traversable {
    ...
    let instantiate: <F>() => <TTraversable extends Traversable<F>>(_: TTraversable) => TTraversable & Ext<F>;
}
...
export { Traversable1 };
declare namespace ITraversable1 {
    let Def: <F extends URI1>(_: Traversable1<F>) => ITraversable1<F> = (<F extends URI1>(TraversableF: Traversable1<F>) => ({
        traverse: <G extends URI1>(ApplicativeG: Applicative1<G>) => <A, B>(f: (_: A) => Kind1<G, B>) => (traversable: Kind1<F, A>) => (assign(TraversableF.fmap(f)(traversable))(_ => TraversableF.sequenceA(ApplicativeG)(_))),
        sequenceA: <G extends URI1>(ApplicativeG: Applicative1<G>) => <A>(traversable: Kind1<F, Kind1<G, A>>) => (assign(TraversableF.traverse(ApplicativeG))(_ => _<Kind1<G, A>, A>(id)(traversable))),
    }));
    interface Ext<F extends URI1> extends IExtTraversable1<F> {
    }
    let Ext: <F extends URI1>(_: Traversable1<F>) => Ext<F> = (<F extends URI1>(TraversableF: Traversable1<F>) => (define<Ext<F>>(Ext => ({
    ...
    let instantiate: <F extends URI1>() => <TTraversable extends Traversable1<F>>(_: TTraversable) => TTraversable & Ext<F> = (() => _ => assign(merge(Def(_), _))(_ => merge(_, Ext(_))));
}
